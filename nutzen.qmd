...
In diesem Schnellstart lernen Sie Schritt für Schritt, wie Sie aus einem Template ein lokales Quarto-Projekt erstellen und einmal rendern. Anschließend nehmen Sie eine kleine Änderung vor, um den grundlegenden Arbeitsablauf kennenzulernen.
...
Zuweilen behält der Browser die Daten im Cache und die Änderungen nach `quarto render` werden angezeigt. Häufig hilft die Tastenkombination {{< kbd Strg >}} + {{< kbd F5 >}} im Browser und die Seite wird vollständig aktualisiert angezeigt.
:::
...
Für das Testen der Auswirkungen von Änderungen der `_quarto.yml` und der `n4o_oer_html.scss` steht in diesem Repository die Seite [Designtest - Änderungen im Branding überprüfen](./data/quarto-designtest.qmd) bereit.
