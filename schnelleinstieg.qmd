...
:white_check_mark: Entwicklungsumgebung: Die NFDI4Objects OER-Templates wurden mit [VSCodium](https://vscodium.com/) als Entwicklungsumgebung und der [Quarto-Erweiterung](https://open-vsx.org/extension/quarto/quarto) entwickelt. Weitere Entwicklunsumgebungen sind der [Quarto-Dokumentation](https://quarto.org/docs/faq/#what-editing-tools-can-i-use-with-quarto) zu entnehmen.
...
Prüfen Sie die Version und die Installation von Quarto, indem Sie in VSCodium ein Terminalfenster öffnen (*Menü - Terminal - Neues Terminal*), auf der Konsole unten im Programmfenster: `quarto --version` eintippen und mit {{< kbd Enter >}} bestätigen.
...
![Screenshot des Template-Repository in GitHub](img/screen_github_zip-download.png)
...
Zuweilen behält der Browser die Daten im Cache und die Änderungen nach `quarto render` werden nicht angezeigt. Häufig hilft die Tastenkombination {{< kbd Strg >}} + {{< kbd F5 >}} (Windows, Linux); {{< kbd cmd >}} + {{< kbd shift >}}{{< kbd R >}} (Mac) im Browser und die Seite wird vollständig aktualisiert dargestellt.
...
**`_oer_metadata.yml`** enthält strukturierte Metadaten zur Beschreibung der Übung als Open Educational Resource:
    ...
    In ihr können zusätzlich sogenannte [Quarto-Shortcodes](https://quarto.org/docs/extensions/shortcodes.html) definiert werden, also benannte Platzhalter, die an zentraler Stelle einmal festgelegt und anschließend im gesamten Projekt wiederverwendet werden.
